...
class POSDataImport(Document):
	...
	def check_existing_master_data(self):
		...
		parent_total_amount = flt(self.total if self.total is not None else self.net_value if self.net_value is not None else self.invoice_total if hasattr(self, "invoice_total") else 0)
		...
		for row in (self.items or []):
			...
			if required > available:
				not_available.append(
					_("Row {0}: Item {1} has insufficient quantity. Available: {2}, Required: {3}.").format(
		...
		for e in others:
			...
			if e.get("docstatus") == 0:
				try:
					...
					previous.db_set("rejected_reason", _("New Import {0} has been submitted for this Invoice").format(self.name))
				except Exception:
					frappe.log_error(f"Failed to mark previous POS Data Import {e.get('name')} as DUPLICATE")
	...
	def create_sales_invoice(self):
		...
		payment_amount = flt(getattr(self, "pay_value", None) or getattr(self, "pay_value_visa", None) or 0.0)
		si.insert()
