    ...
    pos_invoice = db.sql(""" 
        ...
        pos_invoice_collecting AS (
            SELECT 
                ...
                JSON_ARRAYAGG(
                    JSON_OBJECT(
                        'item_code' , JSON_UNQUOTE(JSON_QUOTE(r.item_code)) , 
                        'barcode' , JSON_UNQUOTE(JSON_QUOTE(r.barcode)) , 
                        'item_description' , JSON_UNQUOTE(JSON_QUOTE(r.item_description) ), 
                        'quantity' , JSON_UNQUOTE(JSON_QUOTE(r.quantity)) , 
                        'rate' , JSON_UNQUOTE(JSON_QUOTE(r.rate) ), 
                        'amount' , JSON_UNQUOTE(JSON_QUOTE(r.amount) ), 
                        'discount_value' , JSON_UNQUOTE(JSON_QUOTE(r.row_discount_value)) , 
                        'status' , JSON_UNQUOTE(JSON_QUOTE(r.row_status)) , 
                        'pos_data_check' , JSON_UNQUOTE(JSON_QUOTE(r.pos_data_check) ), 
                        'api_ref' , JSON_UNQUOTE(JSON_QUOTE(r.api_ref)) , 
                        'pos_data_check_status' , JSON_UNQUOTE(JSON_QUOTE(r.pos_data_check_status)) , 
                        'pos_data_check_rejected_reason' , JSON_UNQUOTE(JSON_QUOTE(r.pos_data_check_rejected_reason)) , 
                        'rejected_reason' , JSON_UNQUOTE(JSON_QUOTE(r.row_rejected_reason)) , 
                        'invoice_pk' , JSON_UNQUOTE(JSON_QUOTE(r.invoice_pk)) , 
                        'row_pk' , JSON_UNQUOTE(JSON_QUOTE(r.row_pk))
                    )
                ) AS items
            FROM 
                pos_data_row AS r 
        ...
        SELECT JSON_OBJECT(
                'invoice_pk' , JSON_UNQUOTE(JSON_QUOTE(j.invoice_pk)) , 
                'market_id' , JSON_UNQUOTE(JSON_QUOTE(j.market_id)) , 
                'market_description' , JSON_UNQUOTE(JSON_QUOTE(j.market_description)) , 
                'pos_no' , JSON_UNQUOTE(JSON_QUOTE(j.pos_no)) , 
                'receipt_no' , JSON_UNQUOTE(JSON_QUOTE(j.receipt_no) ), 
                'pos_profile' , JSON_UNQUOTE(JSON_QUOTE(j.pos_profile)) , 
                'posting_date' , JSON_UNQUOTE(JSON_QUOTE(j.posting_date)) , 
                'posting_time' , JSON_UNQUOTE(JSON_QUOTE(j.posting_time)) , 
                'current_year' , JSON_UNQUOTE(JSON_QUOTE(j.current_year)) , 
                'discount_percent' , JSON_UNQUOTE(JSON_QUOTE(j.discount_percent)), 
                'discount_value' , JSON_UNQUOTE(JSON_QUOTE(j.discount_value) ), 
                'payment_method' , JSON_UNQUOTE(JSON_QUOTE(j.payment_method)) , 
                'total' , JSON_UNQUOTE(JSON_QUOTE(j.total) ), 
                'total_quantity' , JSON_UNQUOTE(JSON_QUOTE(j.total_quantity) ), 
                'net_value' , JSON_UNQUOTE(JSON_QUOTE(j.net_value)) , 
                'client_name' , JSON_UNQUOTE(JSON_QUOTE(j.client_name)) , 
                'national_id' , JSON_UNQUOTE(JSON_QUOTE(j.national_id) ), 
                'program_id' , JSON_UNQUOTE(JSON_QUOTE(j.program_id) ), 
                'tid' , JSON_UNQUOTE(JSON_QUOTE(j.tid)) , 
                'rrn' , JSON_UNQUOTE(JSON_QUOTE(j.rrn)) ,
                'auth' , JSON_UNQUOTE(JSON_QUOTE(j.auth)) , 
                'offers_id' , JSON_UNQUOTE(JSON_QUOTE(j.offers_id)) , 
                'refund_receipt_no' , JSON_UNQUOTE(JSON_QUOTE(j.refund_receipt_no)) , 
                'refund_receipt_pos_no'  , JSON_UNQUOTE(JSON_QUOTE(j.refund_receipt_pos_no)) , 
                'cashier_no' , JSON_UNQUOTE(JSON_QUOTE(j.cashier_no)) , 
                'cashier_name' , JSON_UNQUOTE(JSON_QUOTE(j.cashier_name)) , 
                'customer_no' , JSON_UNQUOTE(JSON_QUOTE(j.customer_no) ), 
                'customer_ref' , JSON_UNQUOTE(JSON_QUOTE(j.customer_ref)) , 
                'customer_type' , JSON_UNQUOTE(JSON_QUOTE(j.customer_type)) , 
                'pay_value' , JSON_UNQUOTE(JSON_QUOTE(j.pay_value)), 
                'pay_value_check' , JSON_UNQUOTE(JSON_QUOTE(j.pay_value_check)) , 
                'pay_value_check_no' , JSON_UNQUOTE(JSON_QUOTE(j.pay_value_check_no)) , 
                'pay_value_visa' , JSON_UNQUOTE(JSON_QUOTE(j.pay_value_visa)) , 
                'pay_visa_type' , JSON_UNQUOTE(JSON_QUOTE(j.pay_visa_type)) , 
                'reminder_value' , JSON_UNQUOTE(JSON_QUOTE(j.reminder_value)) , 
                'items' , j.items , 
                'status' , JSON_UNQUOTE(JSON_QUOTE(j.status)) , 
                'rejected_reason' , JSON_UNQUOTE(JSON_QUOTE(j.rejected_reason) )   , 
                'receipt_type' , JSON_UNQUOTE(JSON_QUOTE(j.receipt_type)  )     
            ) as invoice
        FROM pos_invoice j
    """, as_dict=True)
    parent_values = []
