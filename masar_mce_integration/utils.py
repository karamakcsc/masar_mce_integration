    ...
    pos_invoice = db.sql(""" 
        ...
        pos_data_row AS (
            SELECT 
                ...
                c.pos_no ,
                c.receipt_no ,
                CAST( STR_TO_DATE(REPLACE(LEFT(c.date_timestamp,19), 'T', ' '), '%%Y-%%m-%%d %%H:%%i:%%s') AS  	DATE ) AS posting_date,
                CAST( STR_TO_DATE(REPLACE(LEFT(c.date_timestamp,19), 'T', ' '), '%%Y-%%m-%%d %%H:%%i:%%s') AS 	TIME ) AS posting_time,
                c.current_year , 
                c.discount_percent ,
                c.discount_value ,
                c.total , 
